...
const OtherCIty = ({ bg, icon, name, temperatureRange, color, onClick }) => {
  return (
    <div
      className="relative mx-2 w-1/4 max-md:w-5/12 max-md:mt-3 cursor-pointer group"
      onClick={onClick}
    >
      <div
      ...
      >
        <div
          className={`${color} bg-gradient-to-tl h-full rounded-2xl group-hover:opacity-0 transition-opacity duration-300`}
        ></div>
      </div>
